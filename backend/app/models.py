...
class User(Base):
    ...
    profile_attrs = Column(JSON, default=dict)
    crit_weights = Column(JSON, default=dict)
...
class Review(Base):
    ...
    score_seller = Column(Integer)
    score_product = Column(Integer)
...
class Feedback(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.id"))
    review_id = Column(Integer, ForeignKey("reviews.id"))
    is_useful = Column(Integer, default=1)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
